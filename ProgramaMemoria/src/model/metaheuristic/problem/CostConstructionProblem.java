...
import model.metaheuristic.utils.BoundedRandomGenerator;
import model.metaheuristic.utils.JavaRandom;

public class CostConstructionProblem implements Problem<IntegerSolution> {
	private String networkINP;
	...
	public CostConstructionProblem(EpanetAPI epanet, String networkGama) throws IOException, EpanetException {
		if (epanet == null) {
	...
	public void evaluate(IntegerSolution solution) { // Puede ser necesario agregar la excepcion de epanet
		MonoObjetiveSolutionEvaluator evaluator = new MonoObjetiveSolutionEvaluator(30);
		double cost = 0;
	...
	public IntegerSolution createSolution() {
		...
		for (int i = 0; i < getNumberOfVariables(); i++) {
			solution.setVariable(i, random.getRandomValue(this.lowerBound, this.upperBound));
		}
		return solution;
	}
